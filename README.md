...
### Wrap up
Today we learned about the kernel and the bootloader. Although the kernel is what we will be mainly dealing with, it is important for us to understand that the kernel isn't the first code to run on the computer. Code will always run before it, configuring the environment and setting things up. Once the kernel does gain control it ensures everything is ready for the operating system before starting it up.
